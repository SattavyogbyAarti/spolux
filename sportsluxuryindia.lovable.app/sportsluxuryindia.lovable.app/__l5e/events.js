(() => {
  ...
  ((
    ...
    maxKeepaliveBatchBytes = 50 * 1024,
    maxReplayBodyBytes = 512 * 1024,
    maxReplayEventsPerChunk = 100,
    maxReplayTotalBytes = 5 * 1024 * 1024,
    maxReplayChunks = 50,
    maxReplayDurationMs = 10 * 60 * 1e3,
    replayFlushIntervalMs = 2e3
  ) => {
    ...
    const isInternalPath = /* @__PURE__ */ __name(
      (path) => path === "/__codex_token_check" || path.startsWith("/__l5e/"),
      "isInternalPath"
    );
    ...
    const trackUrl =
      script?.getAttribute("data-track-url") || "/__l5e/trackevents";
    const replayUrl =
      script?.getAttribute("data-replay-url") || "/__l5e/replay";
    const replayScriptUrl =
      script?.getAttribute("data-replay-script-url") ||
    ...
    const replaySampleRate = parseSampleRate(
      script?.getAttribute("data-replay-sample-rate") || null
    );
    const replayCrashSampleRate = parseSampleRate(
      script?.getAttribute("data-replay-crash-sample-rate") || null
    );
    const randomId = /* @__PURE__ */ __name(
      () =>
    ...
    const writeRollingSession = /* @__PURE__ */ __name(
      (storage, key, value, expiresAt) => {
        try {
          storage?.setItem(
            key,
            JSON.stringify({ value, expires_at: expiresAt })
          );
        } catch {}
      },
      "writeRollingSession"
    ...
    const anonymousIdentity = storageGetOrCreate(
      w.localStorage,
      "__lovable_anonymous_id"
    );
    const sessionIdentity = loadRollingSession(
      w.localStorage,
      rollingSessionStorageKey
    );
    ...
    const isReplaySampledIn =
      replaySampleRate > 0 && replaySamplingValue < replaySampleRate;
    const isReplayCrashSampledIn =
      replayCrashSampleRate > 0 && replaySamplingValue < replayCrashSampleRate;
    const isReplayRecordingAllowed =
      isReplaySampledIn || isReplayCrashSampledIn;
    const replayEffectiveSampleRate = isReplaySampledIn
      ? replaySampleRate
      : replayCrashSampleRate;
    let replayId = null;
    ...
    const replayQueue = [];
    const maxReplayBufferedEvents = Math.max(
      maxReplayEventsPerChunk,
      maxReplayEventsPerChunk * maxReplayChunks
    );
    ...
    const maskReplayText = /* @__PURE__ */ __name(
    ...
    const projectEventNamePattern =
      /^project\.[a-z][a-z0-9_]{0,63}(\.[a-z][a-z0-9_]{0,63})*$/u;
    ...
    const byteLength = /* @__PURE__ */ __name((value) => {
      ...
      return TextEncoderCtor
        ? new TextEncoderCtor().encode(value).length
        : value.length;
    }, "byteLength");
    const createBatchBody = /* @__PURE__ */ __name(
      (events) =>
        JSON.stringify({
    ...
    const createReplayChunkBody = /* @__PURE__ */ __name(
      (events, chunkID, chunkSequence) => {
    ...
    const canSendReplayEvent = /* @__PURE__ */ __name((event) => {
      const body = createReplayChunkBody(
        [event],
    ...
    const transmitReplayChunkBody = /* @__PURE__ */ __name(
      (body, preferKeepalive = false) => {
        const fetchFn = w.fetch || (typeof fetch === "function" ? fetch : null);
        if (!fetchFn) return;
        const bodyBytes = byteLength(body);
        const useKeepalive =
          preferKeepalive && bodyBytes <= maxKeepaliveBatchBytes;
        fetchFn(replayUrl, {
          method: "POST",
          headers: { "content-type": "application/json" },
          body,
    ...
    const replayDurationMs = /* @__PURE__ */ __name(
      () => Math.max(0, perf.now() - replayStartedAt),
      "replayDurationMs"
    );
    ...
    const emitReplayErrorFor = /* @__PURE__ */ __name(
      (
        targetReplayId,
        reason,
        eventCount,
    ...
    const replayCaptureProperties = /* @__PURE__ */ __name(
      () =>
        !replayCaptureReason || replayCaptureReason === "sampled"
          ? {}
          : {
    ...
    const emitReplayMetadataFor = /* @__PURE__ */ __name(
      (
        targetReplayId,
        status,
        reason,
    ...
    const shouldEmitReplayLifecycleMetadata = /* @__PURE__ */ __name(
      () => replayUploadEnabled || replayCaptureReason === "sampled",
      "shouldEmitReplayLifecycleMetadata"
    );
    const emitReplayMetadataOrErrorFor = /* @__PURE__ */ __name(
      (
        targetReplayId,
        status,
        reason,
    ...
    const stopReplayRecording = /* @__PURE__ */ __name(
      (reason, dropQueuedEvents = false) => {
    ...
    const markReplayCapturedByClientError = /* @__PURE__ */ __name(
      (triggerEventId) => {
        if (
          !replayId ||
          replayFinalized ||
    ...
    const hasReplayDataToCapture = /* @__PURE__ */ __name(
      () =>
        replayQueue.length > 0 ||
        replayChunkSequence > 0 ||
        replayUploadedBytes > 0,
      "hasReplayDataToCapture"
    );
    const emitReplayCaptureMetadataIfReady = /* @__PURE__ */ __name(() => {
      if (
        ...
        replayFinalized ||
        replayCaptureReason !== "client_error" ||
        replayCaptureMetadataEmitted ||
        !replayHasFullSnapshot ||
        !hasReplayDataToCapture()
      ) {
        return false;
      }
    ...
    const flushReplay = /* @__PURE__ */ __name((preferKeepalive = false) => {
      ...
      while (!replayFinalized && replayQueue.length > 0) {
        ...
        if (
          replayChunkSequence >= maxReplayChunks ||
          replayUploadedBytes + bodyBytes > maxReplayTotalBytes
        ) {
          stopReplayRecording(
            replayChunkSequence >= maxReplayChunks
              ? "chunk_limit"
    ...
    const scheduleReplayFlush = /* @__PURE__ */ __name(() => {
      if (replayFlushTimer) return;
      const setTimer =
        w.setTimeout || (typeof setTimeout === "function" ? setTimeout : null);
      if (!setTimer) {
        flushReplay();
        return;
      }
    ...
    const shouldFlushImmediately = /* @__PURE__ */ __name(
      (eventName) =>
        eventName === "lovable.client_error" ||
        eventName === "lovable.resource_error" ||
        eventName === "lovable.replay_error",
    ...
    const refreshSession = /* @__PURE__ */ __name(() => {
      ...
      const sharedSession = readRollingSession(
        w.localStorage,
        rollingSessionStorageKey
      );
    ...
    const queueEvent = /* @__PURE__ */ __name(
      (eventName, eventSchemaVersion, properties = {}) => {
        const activeReplayId =
          replayId &&
          !replayFinalized &&
          (replayUploadEnabled || eventName === "lovable.client_error")
            ? replayId
            : null;
        ...
        if (eventName === "lovable.client_error")
          markReplayCapturedByClientError(eventId);
    ...
    const enqueue = /* @__PURE__ */ __name((eventName, properties = {}) => {
      const eventSchemaVersion =
        typeof eventName === "string" ? schemaVersionFor(eventName) : null;
      if (eventSchemaVersion === null) return null;
      if (eventName !== "lovable.session_started" && refreshSession()) {
        const sessionStartedSchemaVersion = schemaVersionFor(
          "lovable.session_started"
        );
    ...
    const track = /* @__PURE__ */ __name((eventName, properties = {}) => {
      if (eventName === "lovable.page_viewed") {
        rotatePageViewId();
        if (typeof properties.url_path === "string")
          lastPageViewPath = properties.url_path;
      }
    ...
    const serializeThrowable = /* @__PURE__ */ __name((value) => {
      ...
      const message2 =
        stringifyPrimitive(throwable?.message) ??
    ...
    const emitPageView = /* @__PURE__ */ __name(
      (navigationType, rotatePageView = true) => {
    ...
    const emitPageViewIfPathChanged = /* @__PURE__ */ __name(
      (navigationType) => {
    ...
    const startReplayRecording = /* @__PURE__ */ __name(() => {
      const record =
        replayMode === "rrweb" &&
        isReplayRecordingAllowed &&
        w.rrweb &&
        typeof w.rrweb.record === "function"
          ? w.rrweb.record
          : null;
      ...
      try {
        const stop = record({
          emit(event) {
            ...
            while (
              !replayUploadEnabled &&
              replayQueue.length > maxReplayBufferedEvents
            ) {
              const removableIndex = replayQueue.findIndex(
                (queuedEvent) => replayEventType(queuedEvent) !== 2
              );
    ...
    const stopReplayRecordingOnRuntimeError = /* @__PURE__ */ __name(
      (event) => {
        if (!replayId || replayFinalized) return;
        ...
        const stack =
          typeof errorRecord?.stack === "string" ? errorRecord.stack : "";
        const isRecorderError =
          filename.includes("/__l5e/rrweb-record") ||
          stack.includes("/__l5e/rrweb-record");
        if (isRecorderError) {
    ...
    const webVitalUnit = /* @__PURE__ */ __name(
      (metric) => (metric === "CLS" ? "score" : "millisecond"),
      "webVitalUnit"
    );
    const emitWebVital = /* @__PURE__ */ __name(
      (metric, value, delta = value, id2 = randomId()) => {
    ...
    const observePerformanceEntries = /* @__PURE__ */ __name(
      (entryType, callback) => {
        ...
        try {
          const observer = new ObserverCtor((list) => {
            const entries =
              typeof list.getEntries === "function" ? list.getEntries() : [];
          ...
          observer.observe(
            entryType === "event"
              ? { type: entryType, buffered: true, durationThreshold: 40 }
    ...
    const observeWebVitals = /* @__PURE__ */ __name(() => {
      ...
      const reportMetric = /* @__PURE__ */ __name((metric, value) => {
        ...
        emitWebVital(
          metric,
          value,
          previousValue === void 0 ? value : value - previousValue,
          metricId(metric)
        );
      }, "reportMetric");
      ...
      const navigationEntry =
        navigationEntries[0] && typeof navigationEntries[0] === "object"
          ? navigationEntries[0]
          : null;
      const responseStart = navigationEntry
        ? performanceEntryValue(navigationEntry, "responseStart")
        : null;
      const requestStart = navigationEntry
        ? performanceEntryValue(navigationEntry, "requestStart")
        : null;
      const fetchStart = navigationEntry
        ? performanceEntryValue(navigationEntry, "fetchStart")
        : null;
      const ttfbStart = requestStart ?? fetchStart;
      if (
        responseStart !== null &&
        ttfbStart !== null &&
        responseStart >= ttfbStart
      ) {
      ...
      observePerformanceEntries("largest-contentful-paint", (entry) => {
        latestLcp =
          performanceEntryValue(entry, "startTime") ??
          performanceEntryValue(entry, "renderTime") ??
          performanceEntryValue(entry, "loadTime") ??
          latestLcp;
      });
      const clsObserverStarted = observePerformanceEntries(
        "layout-shift",
        (entry) => {
    ...
    const normalizeCaptureExceptionOptions = /* @__PURE__ */ __name(
      (options) => {
        const record =
          options && typeof options === "object" && !Array.isArray(options)
            ...
            : {};
        const mechanism =
          typeof record.mechanism === "string" ? record.mechanism : "manual";
        const severity =
          typeof record.severity === "string" ? record.severity : "error";
        return {
          mechanism: [
            "manual",
            "onerror",
    ...
    w.__lovableEvents = Object.assign(existing, {
      ...
      identify(identity) {
        ...
        appUserId =
          typeof identityRecord?.user_id === "string"
      ...
      getIdentity() {
        return {
          anonymous_id: anonymousId,
          app_user_id_hash_available: !!appUserId,
        };
      },
      getReplay() {
        return {
      ...
      captureException(error, context = {}, options = {}) {
        ...
        let captureContext =
          context && typeof context === "object" && !Array.isArray(context)
            ...
            : {};
        try {
          if (byteLength(JSON.stringify(captureContext)) > 4096)
            captureContext = {};
        } catch {
          captureContext = {};
        }
    ...
    if (w.addEventListener) {
      w.addEventListener(
        ...
        (event) => {
          const errorEvent = event && typeof event === "object" ? event : null;
          const target =
            errorEvent?.target && typeof errorEvent.target === "object"
              ? errorEvent.target
              : null;
          ...
          const throwable = serializeThrowable(
          ...
          enqueue("lovable.client_error", {
            ...
            filename:
              typeof errorEvent?.filename === "string"
                ? pathFromUrl(errorEvent.filename)
                : null,
            lineno:
              typeof errorEvent?.lineno === "number" ? errorEvent.lineno : null,
            colno:
              typeof errorEvent?.colno === "number" ? errorEvent.colno : null,
            mechanism: "onerror",
            handled: false,
            severity: "error",
          });
      ...
      w.addEventListener("popstate", () =>
        emitPageViewIfPathChanged("popstate")
      );
    ...
    window,
    "0.1.2-error-event-hardening",
    {
      "lovable.session_started": 1,
      "lovable.page_viewed": 1,
      "lovable.client_error": 1,
      "lovable.frontend_span": 1,
      "lovable.interaction": 1,
      "lovable.form_submitted": 1,
      "lovable.resource_error": 1,
      "lovable.session_replay": 1,
      "lovable.replay_error": 1,
      "lovable.web_vital": 1,
    },
    50,
    51200,
